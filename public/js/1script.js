document.addEventListener('DOMContentLoaded', () => {
    ...
    // Use a slightly more robust way to get the page name
    ...
    function getDistractors(correctAnswer, answerPool, count = 3) {
        ...
        // If still not enough unique answers after trying to fill up, just take what's available
        poolForDistractors = poolForDistractors.slice(0, count);


        while (distractors.size < count && poolForDistractors.length > 0) {
            const randomIndex = Math.floor(Math.random() * poolForDistractors.length);
            distractors.add(poolForDistractors.splice(randomIndex, 1)[0]);
        }
        // Fallback if still not enough distractors (very rare case, e.g., only one possible answer in the whole pool)
        while (distractors.size < count) {
            distractors.add(`Opsi ${distractors.size + 1}`);
        }
    ...
    function prepareQuizQuestions() {
        ...
        if (!currentScript) {
            console.error('No script selected for quiz preparation.');
        ...
        if (selectedSubCategories.length === 0) {
            console.error('No subcategories selected for quiz preparation.');
            showNotification('Tidak ada subkategori yang dipilih. Pilih minimal satu.', 3000);
            navigateTo('selectcategory'); // Redirect to category selection
            return false; // Indicate failure
        }

        // Ensure quizData is loaded
        if (!quizData || !Array.isArray(quizData) || quizData.length === 0) {
            console.error('Quiz data not available for quiz preparation.');
        ...
        if (!scriptData || !scriptData.categories) {
            console.error('Script data not found or invalid for key:', currentScript);
        ...
        selectedSubCategories.forEach(subCategoryValue => {
            ...
            if (scriptData.categories[mainCategoryKey] &&
                ...
                for (const character in subCategoryData) {
                    const reading = subCategoryData[character];
                    if (reading !== null && reading !== undefined) {
                        allPossibleAnswers.push(reading);
                    }
                }
            }
        });
        ...
        selectedSubCategories.forEach(subCategoryValue => {
            ...
            if (scriptData.categories[mainCategoryKey] &&
                ...
                // subCategoryData is an object { character: reading, ... }
                for (const character in subCategoryData) {
                    const reading = subCategoryData[character];
                    if (character !== null && reading !== null && character !== undefined && reading !== undefined) {
                        // Add question pair
                        rawQuestions.push({ q: character, a: reading });
                    }
                }
            } else {
        ...
        if (rawQuestions.length === 0) {
            console.warn('No questions found in selected subcategories.');
        ...
        for (let i = quizQuestions.length - 1; i > 0; i--) {
            ...
            [quizQuestions[i], quizQuestions[j]] = [quizQuestions[j], quizquisition[i]];
        }

        // Simpan quizQuestions ke localStorage
        localStorage.setItem('quizQuestions', JSON.stringify(quizQuestions));
        console.log(`Prepared ${quizQuestions.length} quiz questions.`);
    ...
    function displayQuestion() {
        ...
        if (currentQuestionIndex >= quizQuestions.length || quizQuestions.length === 0) {
            console.log('Quiz finished or no questions.');
        ...
        console.log(`Displaying question ${currentQuestionIndex + 1}: "${questionData.q}"`);
    }
    ...
    function handleAnswer(event) {
        ...
        if (quizQuestions.length === 0) {
            console.error('Quiz state not loaded in handleAnswer.');
        ...
        if (selectedAnswer === correctAnswer) {
            ...
            if (feedbackArea) {
                ...
                feedbackArea.className = 'feedback-area correct'; // Use class for styling
            }
            ...
            if (feedbackArea) {
                ...
                feedbackArea.className = 'feedback-area incorrect'; // Use class for styling
            }
    ...
    function handleSkip() {
        ...
        // Ensure state is loaded
        if (quizQuestions.length === 0) {
            console.error('Quiz state not loaded in handleSkip.');
            showNotification('Terjadi kesalahan pada quiz. Memuat ulang.', 3000);
            setTimeout(() => navigateTo('selectcategory'), 3000); // Redirect
            return;
        }
    ...
    function loadCategories() {
        console.log('Loading categories UI...');

        // Ensure quizData is available
        if (!quizData || !Array.isArray(quizData) || quizData.length === 0) {
            console.error('Quiz data is not loaded or invalid for category selection.');
        ...
        if (!scriptData || !scriptData.categories) {
            console.error('Data kategori tidak ditemukan untuk script:', scriptKey);
        ...
        console.log('Rendering categories for script:', scriptData.title);
        ...
        if (!mainCategoryContainer) {
            console.error('Main category container (.category-accordion or #main-category-container) not found. Cannot render categories.');
        ...
        Object.entries(scriptData.categories).forEach(([categoryKey, category]) => {
            if (category && typeof category === 'object' && category.name && category.subCategories && typeof category.subCategories === 'object') {
                ...
                Object.entries(category.subCategories).forEach(([subCategoryKey, questions]) => {
                    // subCategory here is the questions object { char: reading, ... }
                    if (questions && typeof questions === 'object' && Object.keys(questions).length > 0) { // Only add subcategory if it has questions
                        ...
                        subCategoryItem.innerHTML = `
                            <label for="${checkboxId}">
                                ...
                                <span>${subCategoryKey}</span> </label>
                        `;
                        subOptionsGrid.appendChild(subCategoryItem);
                    } else {
                        console.warn(`Subcategory key "${subCategoryKey}" in category "${categoryKey}" has no questions.`);
                    }
                });
                ...
                if (subOptionsGrid.children.length > 0) {
                    ...
                    console.warn(`Category "${category.name}" has no subcategories with questions. Skipping.`);
                }

            } else {
                console.warn(`Skipping invalid category data for key: ${categoryKey}`, category);
            }
        });
        ...
        console.log('Category UI rendering complete.');
    ...
    function speakText(text) {
        if (!synth || !japaneseVoice) {
            ...
            showNotification('Fitur suara tidak siap.', 2000);
            return;
        }
    ...
    function addPulseAnimation(element) {
        ...
        // Make sure this timeout matches your CSS animation duration
        setTimeout(() => {
    ...
    function loadLearningData(scriptKey) {
        ...
        if (!quizData || !Array.isArray(quizData) || quizData.length === 0) {
            ...
            console.error('Quiz data is not loaded or invalid for learning page.');
        ...
        if (!scriptData) {
            ...
            console.error('Script data not found for learning:', scriptKey);
            ...
            Object.entries(scriptData.categories).forEach(([categoryKey, category]) => {
                if (category && typeof category === 'object' && category.name && category.subCategories && typeof category.subCategories === 'object') {
                        ...
                        if (questions && typeof questions === 'object' && Object.keys(questions).length > 0) {
                            ...
                            Object.entries(questions).forEach(([character, readingData]) => {
                                ...
                                if (typeof readingData === 'string') {
                                    readingText = readingData;
                                } else if (typeof readingData === 'object' && readingData !== null) {
                                    readingText = readingData.reading || ''; // Assuming { reading: '...', meaning: '...' } structure for Kanji
                                ...
                                // --- REMOVE SPEAK BUTTON CREATION HERE ---
                                // The button creation code should be deleted
                            ...
                            if (characterGrid.children.length > 0) {
                                ...
                                console.warn(`Subcategory key "${subCategoryKey}" in category "${categoryKey}" resulted in no characters.`);
                            }
                        } else {
                            console.warn(`Subcategory key "${subCategoryKey}" in category "${categoryKey}" has no questions or invalid format.`);
                        }
                    });
                    ...
                    if (sectionHasContent) {
                        ...
                        console.warn(`Category "${category.name}" has no subcategories with valid characters. Skipping section.`);
                    }
        ...
        console.log('Learning data rendered with clickable items.');
        ...
        console.log('Speak button listeners no longer needed, clicks handled by item.');
        ...
        //         }
        //     });
        //      console.log('Learning script tab active state updated.');
        // }
    }
    ...
    async function initializePage() {
        ...
        switch (currentPage) {
            case 'index':
                ...
                break;
            case 'selectscript':
                console.log('Initializing Select Script page...');
                initializeSelectScriptPage();
                break;
            case 'selectcategory':
                console.log('Initializing Select Category page...');
    ...
    function disableDataDependentFeatures() {
        console.warn('Data fetch failed or data is invalid. Disabling data-dependent features.');
        ...
        const dataDependentElements = document.querySelectorAll(
            ...
            '.speak-button, ' + // Buttons on learning page
    ...
    function initializeIndexPage() {
        console.log('Index page specific setup.');
        ...
        if (startQuizButton) {
            ...
            console.warn('Start Quiz button not found on index page.');
        }
        ...
        if (startLearningButton) {
            ...
            console.log('Start Learning button listener added.');
        } else {
            console.warn('Start Learning button not found on index page.');
        }
    ...
    function handleStartQuiz() {
        console.log('Start Quiz button clicked. Navigating to selectscript.');
    ...
    function handleStartLearning() {
        console.log('Start Learning button clicked. Navigating to learning.');
    ...
    function initializeSelectScriptPage() {
        console.log('Select Script page specific setup.');
                ...
                scriptSelectButtons.forEach(button => {
                    ...
                    if (!button.dataset.listenerAdded) {
                        ...
                        button.addEventListener('click', () => {
                            console.log('Script selected:', scriptKey);
                        ...
                        console.log(`Listener added for script button: ${scriptKey}`);
                    }
                });
                console.log('Script selection button listeners added.');
            } else {
                console.warn('No .btn-script buttons found within the container.');
            }

        } else if (quizData.length === 0) {
            console.warn('No quiz data loaded, script selection buttons may be disabled.');
    ...
    function initializeSelectCategoryPage() {
        console.log('Select Category page specific setup.');
        ...
        if (mainCategoryContainer) {
            ...
            console.log('Subcategory checkbox listener added via delegation.');
        } else {
            console.warn('Main category container not found, checkbox listener not attached.');
        ...
        if (startGameButton) {
            ...
            console.warn('Start game button not found on selectcategory page.');
            // disableDataDependentFeatures already called if data fetch failed
        }
    ...
    function handleStartGame() {
        ...
        if (selectedSubCategories.length > 0) {
            ...
            const questionsPrepared = prepareQuizQuestions(); // This saves to localStorage
            if (questionsPrepared) {
                ...
                // prepareQuizQuestions would have already shown a notification
                console.warn('Quiz preparation failed or resulted in zero questions.');
            }
        } else {
    ...
    function initializeGamePage() {
        console.log('Game page specific setup.');
        ...
        if (quizQuestions.length === 0) {
            console.warn('No quiz questions found in localStorage. Attempting to prepare questions based on last selection.');
            ...
            if (selectedScript && selectedSubCategories.length > 0 && quizData.length > 0) {
                ...
                if (questionsPrepared) {
                    ...
                    console.log('Quiz questions re-prepared and state reset.');

                } else {
                    console.error('Cannot prepare quiz questions: preparation failed.');
                    showNotification('Gagal memuat soal quiz.', 4000);
                    navigateTo('selectscript'); // Redirect if cannot prepare
                    return; // Stop initialization
                }
            } else {
                console.error('Cannot prepare quiz questions: missing script selection, subcategories, or quiz data.');
        ...
        if (quizQuestions.length === 0) {
            console.error('Failed to load or prepare quiz questions.');
            showNotification('Gagal memuat soal quiz.', 4000);
        ...
        if (quitGameButton) {
            ...
            console.log('Quit game button listener added.');
        } else {
            console.warn('Quit game button not found on game page.');
        }
        ...
        if (skipButton) {
            ...
            console.warn('Skip button not found on game page.');
    ...
    function initializeResultPage() {
        console.log('Result page specific setup.');
        ...
        if (tryAgainButton) {
            ...
            console.log('Try Again button listener added.');
        } else {
            console.warn('Try Again button not found on result page.');
        }
        ...
        if (newCategoriesButton) {
            ...
            console.log('New Categories button listener added.');
        } else {
            console.warn('New Categories button not found on result page.');
        }
        ...
        if (homeButton) {
            ...
            console.warn('Home button not found on result page.');
        }
    }
    ...
    function handleNewCategories() {
        console.log('Select New Categories clicked.');
    ...
    function initializeLearningPage() {
        console.log('Learning page specific setup.');
            ...
            if (learningScriptButtons.length > 0) {
                learningScriptButtons.forEach(button => {
                    ...
                    if (!button.dataset.listenerAdded) {
                        const scriptKey = button.dataset.script;
                        button.addEventListener('click', () => {
                            console.log('Learning script selected via tab:', scriptKey);
                        ...
                        button.dataset.listenerAdded = 'true'; // Mark as having listener
                        console.log(`Listener added for learning script button: ${scriptKey}`);
                    }
                });
                console.log('Learning script selection button listeners added.');
            } else {
                console.warn('No .btn-script buttons found within the script tabs container.');
            }

        } else if (quizData.length === 0) {
            console.warn('No quiz data loaded, learning script selection buttons may be disabled.');
            // disableDataDependentFeatures already called by main initializer
        }
